...
def movimientos(usu,con):
    ...
    while(lee!=''):
        ...
        if(aux2 == num_t):
            cont3+=1
            print("**********************************************************************************")
            print("N° de tarjeta:",str(aux[0])+"|"+str(aux[1])+"|"+str(aux[2])+"|"+str(aux[3])+"|"+str(aux[4])+"|"+str(aux[5]))
        lee=archivo3.readline()
...
#------------------------------------------------------------[ej1 tren]-----------------------------------
def viaje(lineas,lsm,luq,lsa,lM1,lM2,lM3,lBN,lBS1,lBS2,lR1,lR2,lR3,usu,con):
    archivo = open("sube_datos.txt", "r")
    lee = archivo.readline()
    while lee != '':
        lee_aux = lee.split(',')
    ...
    if float(saldo) >= 100:
        ...
        for i in range(len(lineas)):
            ...
            cont+=1
        print("0 Salir")
        opc = int(input("Seleccione la linea en la que desea viajar: "))
        if opc == 1:#san martin-------------------------------
            est(lsm,usu,con,opc-1,lineas)
        elif opc == 2:#urquiza----------------------------------
            est(luq,usu,con,opc-1,lineas)
        elif opc == 3:#sarmiento-------------------------------------
            est(lsa,usu,con,opc-1,lineas)
        elif opc == 4:#mitre-------------------------------------------------------------------------------------------------------------
            print("1 Ramal Tigre (Retiro - Tigre)\n2 Ramal Suarez (Retiro - José León Suarez)\n3 Ramal Mitre (Retiro - Mitre)\n4 Salir")
            opc_ramal = int(input("En que ramal de la linea Mitre viajara: "))
            while opc_ramal>3 or opc_ramal<0:
                opc_ramal=int(input("Ingrese una opcion valida: "))
            if opc_ramal == 1:
                est(lM1,usu,con,opc-1,lineas)
            elif opc_ramal == 2:
                est(lM2,usu,con,opc-1,lineas)
            elif opc_ramal == 3:
                est(lM3,usu,con,opc-1,lineas)
            elif opc_ramal == 0 or opc_ramal >= 4:
                print("")
        
        elif opc == 5:#belgrano norte------------------------------------------------------------------------------------
            est(lBN,usu,con,opc-1,lineas)
        elif opc == 6:#belgrano sur-----------------------------------------------------------------------------------------------------
            ...
            opc_ramal = int(input("En que ramal de la linea Belgrano Sur viajara: "))
            while opc_ramal>2 or opc_ramal<0:
                opc_ramal=int(input("Ingrese una opcion valida: "))
            if opc_ramal == 1:
                est(lBS1,usu,con,opc-1,lineas)
            elif opc_ramal == 2:
                est(lBS2,usu,con,opc-1,lineas)
            elif opc_ramal == 0 or opc_ramal >= 3:
                print()
        elif opc == 7:#roca----------------------------------------------------------------------------------------------------------------------
            ...
            opc_ramal = int(input("En que ramal de la linea Roca viajara: "))
            while opc_ramal>3 or opc_ramal<0:
                opc_ramal=int(input("Ingrese una opcion valida: "))
            if opc_ramal == 1:
                est(lR1,usu,con,opc-1,lineas)
            elif opc_ramal == 2:
                est(lR2,usu,con,opc-1,lineas)
            elif opc_ramal == 3:
                est(lR3,usu,con,opc-1,lineas)
            elif opc_ramal == 0 or opc_ramal >= 4:
                print("")
        elif opc == "0":
            print("")
    else:
        print("ERROR \nEl saldo minimo requerido para realizar un viaje es $100")
        menu(usu,con)
#----------------------------------
def est(est,usu,con,opc,lineas):
    ...
    if baja > sube:
        ...
        for i in range(estaciones):
            if (i+1)%5 == 0:
                total+=Xest
        cobro = total+min
    print("El valor del boleto es de $",cobro)
...
#---------------------------------------------------------------resta
    ...
    while le != "":
        ...
        le = archivo.readline()
    archivo.close()
    saldo =float(float(sal)-float(cobro))
...
#---------------------------------------------------------datos al movimiento
    ...
    while lee != '':
        lee_aux = lee.split(',')
        if lee_aux[5] == usu and lee_aux[6] == con:
            auxiliar = lee_aux[7]+","+estacion_f+","+"Gasto: $"+str(cobro)+","+"Saldo: $"+str(saldo)+","+"Cobro Total: $"+str(max)+","+"Devuelve: $"+str(max-cobro)+"\n"
...
def arch_m1(usu,con):
    ...
    viaje(lineas,lsm,luq,lsa,lM1,lM2,lM3,lBN,lBS1,lBS2,lR1,lR2,lR3,usu,con)
#--------------------------------[menu+login]----------------------------------------------------------------------------------
def menu(usu,con):
    print("--------------------------[menu]---------------------------")
    ...
    while lee != '':
        ...
        if(str(lee2[9])!='X'):
            ...
            if(str(lee2[5])==usu):
                ...
                print("no se a podido iniciar sesion..\n intentelo de nuevo")
                login()
        else:
            verif=False
        lee=archivo.readline()
